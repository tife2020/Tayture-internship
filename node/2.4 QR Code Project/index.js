...
.then((answer) => {
    ...
    qr_svg.pipe(fs.createWriteStream("qrcodeSvg.svg"));
    ...
    qr_svg.pipe(fs.createWriteStream("qrcodePng.png"));
    ...
    fs.writeFile('qrcodePngMeth2.svg', svg_string , (err) => {
        if (err) throw err;
    ...
    // cretinng the png file and writing the string png data as the file's content
    fs.writeFile('qrcodePngMeth2.png', png_string , (err) => {
        if (err) throw err;
